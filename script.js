...
/* Table handling */
const itemsBody = q('#items_body'), rowsCount = q('#rows_count');
...
q('#clear_table').addEventListener('click', ()=>{ if(!confirm('Clear all rows?')) return; itemsBody.innerHTML=''; rowsCount.textContent=0; updateAll(); });
...
function parseTime(text){
  ...
  let m;
  if(m = t.match(/^(\d{1,2}):(\d{2})\s*(am|pm)?$/)){ let hh=+m[1], mm=+m[2], ap=m[3]; if(ap){ if(ap==='pm' && hh<12) hh+=12; if(ap==='am' && hh===12) hh=0; } return {h:hh,m:mm}; }
  if(m = t.match(/^(\d{1,2})\s*(am|pm)$/)){ let hh=+m[1], ap=m[2]; if(ap==='pm' && hh<12) hh+=12; if(ap==='am' && hh===12) hh=0; return {h:hh,m:0}; }
  if(m = t.match(/^(\d{1,2}):(\d{2})$/)){ return {h:+m[1], m:+m[2]}; }
  return null;
}
...
function updateAll(){
  ...
  window._calc = { ord, aft, sat, sun, totalKm, gross, travel, reimb, subtotal, gstAmount, totalWithGst, superRate, superContribution, bankPayable };
}

/* watch inputs */
['#rate_ordinary','#rate_afternoon','#rate_saturday','#rate_sunday','#travel_total','#reimb_total','#super_rate','#gst_toggle'].forEach(id=> q(id).addEventListener('input', updateAll));
updateAll();
...
function buildInvoiceHTML(template, accent, fontFamily){
  ...
  const totalRowsHtml = `
    <tr><td>Ordinary Hours (${fmt(c.ord)} * ${fmt(Number(q('#rate_ordinary').value||0))})</td><td class="right">$${fmt(c.ord * Number(q('#rate_ordinary').value||0))}</td></tr>
    <tr><td>Afternoon Hours (${fmt(c.aft)} * ${fmt(Number(q('#rate_afternoon').value||0))})</td><td class="right">$${fmt(c.aft * Number(q('#rate_afternoon').value||0))}</td></tr>
    <tr><td>Saturday (${fmt(c.sat)} * ${fmt(Number(q('#rate_saturday').value||0))})</td><td class="right">$${fmt(c.sat * Number(q('#rate_saturday').value||0))}</td></tr>
    <tr><td>Sunday (${fmt(c.sun)} * ${fmt(Number(q('#rate_sunday').value||0))})</td><td class="right">$${fmt(c.sun * Number(q('#rate_sunday').value||0))}</td></tr>
    <tr><td>Travels</td><td class="right">$${fmt(c.travel)}</td></tr>
    <tr><td>Reimbursement</td><td class="right">$${fmt(c.reimb)}</td></tr>
    <tr style="font-weight:700"><td>Subtotal</td><td class="right">$${fmt(c.subtotal)}</td></tr>
    ${ gstOn ? `<tr><td>GST (10%)</td><td class="right">$${fmt(c.gstAmount)}</td></tr>` : '' }
...
['#template_select','#accent_color','#font_select','#gst_toggle'].forEach(id=> q(id).addEventListener('change', ()=> updateAll()));
